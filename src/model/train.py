...
# Project imports (canonical)
from src.model import model_defs as mdefs
from src.model import model_utils as mutils
...
class RunResult:
    ...
    used_idx: Optional[List[int]] = None  # row indices used to fit
...
class ElasticNetResult(NamedTuple):
    ...
    rows: List[Dict[str, Any]]
    n_obs: int
    used_idx: Optional[List[int]] = None
...
def _summarize_sm_results_fallback(sm_obj: Any, cols: List[str], n_obs: int, model_name: str) -> List[Dict[str, Any]]:
    ...
    for c in cols:
        val = params.get(c, np.nan) if isinstance(params, (dict, pd.Series)) else (np.nan)
        rows.append({
            "model": model_name,
            "term": str(c),
            "coef": float(val) if not pd.isna(val) else float("nan"),
            "std_err": float(bse.get(c, np.nan)) if c in bse else float("nan"),
            "pvalue": float(pvalues.get(c, np.nan)) if c in pvalues else float("nan"),
            "n_obs": int(n_obs)
        })
    return rows
...
def run_ols(
    ...
    return RunResult(model_name=model_name, terms=X.columns.tolist(), n_obs=n_obs, rows=rows, fitted=res, used_idx=list(sdf.index))
...
def run_fixed_effects(
    ...
    LOG.info("DEBUG FE DESIGN COLS: %s", list(X_df.columns)[:60])
    ...
    return RunResult(model_name="FE", terms=cols, n_obs=len(X_arr), rows=rows, fitted=res, used_idx=list(full_idx))
...
def run_elasticnet(df: pd.DataFrame,
    ...
    return ElasticNetResult(fitted=pipeline, rows=rows, n_obs=int(X.shape[0]), used_idx=list(subset.index))
...
def save_artifacts(models_dir: Path,
                   ...
                   fe_X_design_columns: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    try:
        # canonical name for EN pipeline used by scripts
    ...
    # If pipeline contains StandardScaler, persist scaler and meta
    try:
        if en_pipeline_obj is not None and hasattr(en_pipeline_obj, "named_steps"):
            scaler = en_pipeline_obj.named_steps.get("standardscaler") or en_pipeline_obj.named_steps.get("StandardScaler") or en_pipeline_obj.named_steps.get("scaler")
            if scaler is not None:
                try:
                    joblib_dump(scaler, artifacts_dir / "scaler.joblib")
                    meta = {
                        "scale_": None if getattr(scaler, "scale_", None) is None else list(getattr(scaler, "scale_")),
                        "mean_": None if getattr(scaler, "mean_", None) is None else list(getattr(scaler, "mean_")),
                        "n_features_in_": getattr(scaler, "n_features_in_", None)
                    }
                    with open(artifacts_dir / "scaler_meta.json", "w", encoding="utf8") as fh:
                        json.dump(meta, fh, indent=2)
                    results["saved"]["scaler.joblib"] = str(artifacts_dir / "scaler.joblib")
                    results["saved"]["scaler_meta.json"] = str(artifacts_dir / "scaler_meta.json")
                except Exception as e:
                    results["errors"]["scaler_save"] = repr(e)
    except Exception as e:
        results["errors"]["scaler_inspect"] = repr(e)

    # FE param map (try to map param names -> original X_design columns)
    try:
        fe_map = {}
        if fe_res_obj is not None:
            # try to read param names from statsmodels object if available
            param_names = None
            try:
                if hasattr(fe_res_obj, "model") and hasattr(fe_res_obj.model, "data") and getattr(fe_res_obj.model.data, "param_names", None):
                    param_names = list(getattr(fe_res_obj.model.data, "param_names"))
                else:
                    try:
                        # params might be a pandas Series
                        param_names = list(fe_res_obj.params.index)
                    except Exception:
                        param_names = None
            except Exception:
                param_names = None

            # If we have the design columns from FE RunResult (fe_X_design_columns), prefer them
            if fe_X_design_columns:
                # fe_X_design_columns should list design columns in order (const, predictors, FE_dummies)
                # Map param_names if available, else emit simple xN mapping.
                if param_names and len(param_names) == len(fe_X_design_columns):
                    for pn, mapped in zip(param_names, fe_X_design_columns):
                        fe_map[pn] = mapped
                else:
                    # param_names unknown -> create x0/x1 style mapping
                    for i, mapped in enumerate(fe_X_design_columns):
                        pn = "const" if i == 0 else f"x{i}"
                        fe_map[pn] = mapped
            else:
                # fallback: try to use param_names directly as mapping to themselves
                if param_names:
                    for pn in param_names:
                        fe_map[pn] = pn

        if fe_map:
            with open(artifacts_dir / "fe_param_map.json", "w", encoding="utf8") as fh:
                json.dump(fe_map, fh, indent=2)
            results["saved"]["fe_param_map.json"] = str(artifacts_dir / "fe_param_map.json")
    except Exception as e:
        results["errors"]["fe_param_map"] = repr(e)

    # train sample index & info: combine used indices across models (if available)
    try:
        # best-effort; caller may write train_index separately; skip here
        pass
    except Exception:
        pass

    # Write manifest into reports_dir if available else artifacts_dir
    try:
        manifest = {
        ...
        if reports_dir is not None:
            try:
                ...
                results["saved"]["manifest"] = str(reports_dir / "model_artifacts_manifest.json")
            except Exception:
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    # Resolve baseline predictors (robust)
    ...
    try:
        ...
        if not baseline:
            raise ValueError("mdefs.safe_select_columns returned empty")
    except Exception as _e:
        LOG.warning("Baseline resolution via safe_select_columns failed: %s — falling back.", _e)
        baseline = []
        if isinstance(baseline_requested, (list, tuple)):
            for entry in baseline_requested:
                if isinstance(entry, str) and entry in df.columns:
                    baseline.append(entry)
                elif isinstance(entry, dict):
                    for k in ("name", "predictor", "term", "feature"):
                        v = entry.get(k)
                        if isinstance(v, str) and v in df.columns:
                            baseline.append(v)
                            break
        elif isinstance(baseline_requested, dict):
            preds = baseline_requested.get("predictors") or baseline_requested.get("features") or []
            for p in preds:
                if isinstance(p, str) and p in df.columns:
                    baseline.append(p)
    baseline = [b for i, b in enumerate(baseline) if b not in baseline[:i]]
    if not baseline:
        LOG.warning("No baseline predictors available after fallback — proceeding with empty baseline.")

    # save training mask / index (rows with full-case baseline + target)
    df.reset_index(drop=True, inplace=True)
    try:
        train_index_path = artifacts_dir / "train_index.csv"
        baseline_mask = _dropna_for_columns(df, baseline + [cfg["target"]["name"]]).index.to_series()
        baseline_mask.to_csv(train_index_path, index=False, header=False)
        LOG.info("Saved training index -> %s", train_index_path)
    except Exception:
        LOG.debug("Could not write train index (continuing).")
    ...
    # Collect objects & sample indices for reproducibility artifacts
    try:
        ...
        # feature names: prefer baseline order; fall back to pipeline inspection
        _feature_names = None
        try:
            if isinstance(baseline, list) and baseline:
                ...
                if _en_obj is not None and hasattr(_en_obj, "named_steps"):
                    last = list(_en_obj.named_steps.items())[-1][1]
                    if hasattr(last, "feature_names_in_"):
                        _feature_names = list(getattr(last, "feature_names_in_"))
        except Exception:
            _feature_names = None

        # fe design columns (if available from FE RunResult)
        fe_X_cols = None
        try:
            if fe_res is not None and isinstance(fe_res.terms, list):
                fe_X_cols = fe_res.terms
        except Exception:
            fe_X_cols = None

        save_results = save_artifacts(models_dir=models_dir, artifacts_dir=artifacts_dir, reports_dir=reports_dir,
                                     ols_res_obj=_ols_obj, fe_res_obj=_fe_obj, en_pipeline_obj=_en_obj,
                                     feature_names=_feature_names, fe_X_design_columns=fe_X_cols)
