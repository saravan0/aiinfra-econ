...
FEATURES = PROCESSED / "features_lean.csv"
...
def write_summary_md(coverage_df: pd.DataFrame, by_year_df: pd.DataFrame, by_country_df: pd.DataFrame, out_path: Path):
    # derive total rows from the features source to provide an accurate count
    try:
        total_rows = len(pd.read_csv(FEATURES, low_memory=False))
    except Exception:
        total_rows = None
    ...
    if total_rows is not None:
        ...
        lines.append("- total rows (observations): **n/a**\n")
